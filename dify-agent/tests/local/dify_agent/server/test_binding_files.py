...
async def test_real_read_script_handles_utf8_truncation_at_multibyte_boundary(tmp_path: Path) -> None:
    ...
    smile_result = await service.read_file(
        BindingFileReadRequest(backend_binding_ref="binding-ref", path="smile.txt", max_bytes=2)
    )
    cjk_result = await service.read_file(
        BindingFileReadRequest(backend_binding_ref="binding-ref", path="cjk.txt", max_bytes=2)
    )
    ...
    assert commands.deletes == ["local-job-1", "local-job-2", "local-job-3", "local-job-4"]
